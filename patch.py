...
with open('saleshub/app.js', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Remove renderDetailProperties calls from early on
content = re.sub(r"if \(typeof window\.renderDetailProperties === 'function'\) \{\s*window\.renderDetailProperties\(\);\s*\}", "", content)

# 2. Append renderDetailProperties call at the end of the DOMContentLoaded block
# We know the file ends with:
# setTimeout(initForecastModule, 100);
# });
content = content.replace("setTimeout(initForecastModule, 100);\n});", "setTimeout(initForecastModule, 100);\n  if (typeof window.renderDetailProperties === 'function') { window.renderDetailProperties(); }\n});")

# 3. Inject pagination code right after DOMContentLoaded starts
pagination_code = '''
  // --- GLOBALS AND UTILS ---
...
content = re.sub(r"document\.addEventListener\('DOMContentLoaded', \(\) => \{\r?\n", "document.addEventListener('DOMContentLoaded', () => {\n" + pagination_code, content, count=1)


# 4. renderContacts
new_render_contacts = """function renderContacts(filter = 'all') {
    if (!tbody) return;
    try {
...
content = re.sub(r"function renderContacts\(filter = 'all'\) \{.*?updateContactActions\(\);\s*\}", new_render_contacts, content, flags=re.DOTALL)

# 5. renderCompanies
new_render_companies = """function renderCompanies(filter = 'all', query = '') {
    if (!compTbody) return;
    try {
...
content = re.sub(r"function renderCompanies\(filter = 'all', query = ''\) \{.*?updateCompanyActions\(\);\s*\}", new_render_companies, content, flags=re.DOTALL)

# 6. renderDeals
new_render_deals = """function renderDeals() {
...
content = re.sub(r"function renderDeals\(\) \{.*?dealsTbody\.innerHTML \+= trHtml;\s*\}\s*\}", new_render_deals, content, flags=re.DOTALL)

# 7. renderBids
new_render_bids = """function renderBids() {
...
content = re.sub(r"function renderBids\(\) \{.*?countEl\.textContent = \$\{filtered\.length\} bids;\s*\}", new_render_bids, content, flags=re.DOTALL)

with open('saleshub/app.js', 'w', encoding='utf-8') as f:
    f.write(content)
print('Done patching')
